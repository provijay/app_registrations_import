...
```mermaid
flowchart TD
    A[Start: Business SLA Requirement] --> B{Availability Requirement}
    
    B -->|< 99.9%| C[Single Region OK<br>(Use Standard SKU where possible)]
    B -->|99.9% - 99.95%| D[Zone-Redundant Deployment<br>(ZRS, Availability Zones)]
    B -->|>= 99.95%| E{Mission Critical?}
    
    E -->|No| D
    E -->|Yes| F{Is RTO < 4 hours?}
    
    F -->|No| G[Geo-Redundant (RA-GRS, Cross-Region DR)]
    F -->|Yes| H[Active-Active Multi-Region<br>(Front Door / Traffic Manager)]
    
    G --> I[Document SLA & DR Strategy]
    H --> I
    D --> I
    C --> I
    
    I --> J[Ongoing Monitoring & Review]
